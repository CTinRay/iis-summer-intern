...
def k_fold_cross_validation(df, prefix, args):
	...
	K = len(k_fold_index_filename)
	metric = 0
	assert K == 5
	for i in range(0, K):
		# Initialize
		train = preprocessor.load(prefix + "train" + str(i) + ".csv")
		valid = preprocessor.load(prefix + "valid" + str(i) + ".csv")
		# Train
...
def write_to_csv(k_df_list, prefix):
	for i, valid_df in enumerate(k_df_list):
		valid_df.to_csv(prefix + "valid"+ str(i) + ".csv")
		train_df_list = []
		for j, train_df in enumerate(k_df_list):
			if i != j:
				train_df_list.append(train_df)
		train_df_list = pd.concat(train_df_list)
		train_df_list.to_csv(prefix + "train" + str(i) + ".csv")
...
def index_to_dataframe(k_fold_index_list, df):
	...
	for k, index_list in enumerate(k_fold_index_list):
		...
		for i in index_list:
			data['Body'].append(df['Body'][i])
			data['Question'].append(df['Question'][i])
			data['Operand'].append(df['Operand'][i])
